...
impl RestirGiPass {
    pub fn new(resolution: UVec2, device: &wgpu::Device) -> Self {
        ...
        let intermediate_reservoirs = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("appearance-path-tracer-gpu::restir_gi_pass intermediate_reservoirs"),
    ...
    pub fn encode(
        &self,
        parameters: &RestirGiPassParameters,
        device: &wgpu::Device,
    ...
    fn encode_temporal(
        ...
        let shader = pipeline_database.shader_from_src(
            device,
            include_shader_src!(
                "crates/appearance-path-tracer-gpu/assets/shaders/restir_gi_temporal.wgsl"
            ),
        );
        let pipeline = pipeline_database.compute_pipeline(
            device,
            wgpu::ComputePipelineDescriptor {
                label: Some("appearance-path-tracer-gpu::restir_gi_temporal"),
        ...
        let constants = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("appearance-path-tracer-gpu::restir_gi_temporal constants"),
    ...
    fn encode_spatial(
        ...
        let shader = pipeline_database.shader_from_src(
            device,
            include_shader_src!(
                "crates/appearance-path-tracer-gpu/assets/shaders/restir_gi_spatial.wgsl"
            ),
        );
        let pipeline = pipeline_database.compute_pipeline(
            device,
            wgpu::ComputePipelineDescriptor {
                label: Some("appearance-path-tracer-gpu::restir_gi_spatial"),
                ..wgpu::ComputePipelineDescriptor::partial_default(&shader)
            },
            || {
                device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
        ...
        for i in 0..parameters.spatial_pass_count {
            let constants = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
                label: Some("appearance-path-tracer-gpu::restir_gi_spatial constants"),
            ...
            parameters.scene_resources.material_pool().bind_group(
                ...
                |material_pool_bind_group| {
                    ...
                    cpass.set_bind_group(4, &parameters.gbuffer.bind_group(device), &[]);
                    cpass.insert_debug_marker("appearance-path-tracer-gpu::restir_gi_spatial");
